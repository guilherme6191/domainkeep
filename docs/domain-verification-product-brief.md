...
- **A newer proof of DNS control moves the domain.** The previous holder sees a superseded claim and can recover with a new challenge. Guidance asks them to check with whoever manages the domain first, since shared DNS access permits transfers in either direction.
- **Disclose a transfer, never an identity.** Every active pending claim has the same takeover warning, whether held elsewhere or not. An existing association is disclosed only after proof. The winner sees a ten-minute transfer note; the previous holder's superseded explanation persists.
