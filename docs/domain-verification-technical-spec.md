...
The table shows a short form of the value: the prefix, then the first and last ten hex digits of the token, enough to distinguish tokens. The copy action uses the complete value. If clipboard access fails, a toast explains the failure and an anchored popover exposes the full value for manual selection.
...
**List presentation.** One badge component gives every state its own label: `verified` → Verified, `setup_required` → Unchecked, `record_not_found` → Record not found, `value_mismatch` → Wrong value, `temporary_dns_error` → DNS didn’t answer, `held_by_another` → Held elsewhere, `expired` → Code expired, `superseded` → Moved away. A second map gives each state one imperative next step — add the record then verify; wait then check again; fix the value then check again; check again; take over or leave it; get a new code; get a new code to reclaim; nothing for `verified` — which the list row, the detail page’s progress line and its outcome panels all read, so the page never renames the step the list named. A predicate marks the four states a further lookup can still change (`setup_required`, `record_not_found`, `value_mismatch`, `temporary_dns_error`) and decides where a check action is offered. None of this affects state derivation.
